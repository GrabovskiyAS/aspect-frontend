...
const submission = async () => {
  ...
  insertDataReductors(`/data/DeliveryToChinaBorders`, payload)
    .then((response: AxiosResponse) => {
      router.push(`/reductors/DeliveryToChinaBorders/Update/${response.data.id}`)
    })
    .catch((error) => {
      console.log(error)
    })
...
<template>
  <h1 class="pt-5">Доставка до границы Китая. Создание</h1>
  <div class="pt-5">
