...
const submission = async () => {
  ...
  deleteDataReductors(`/data/RedMountTypes/${props.id}`)
    .then(() => {
      router.push('/reductors/RedMountTypes/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
<template>
  ...
  <h1 class="pt-5">RedMountTypes. Удалить?</h1>
  ...
  <div v-else class="pt-5">
    ...
    <div class="field pt-5">
      <FloatLabel>
        <InputText id="image" v-model="data.data[0].image" disabled class="w-full" />
