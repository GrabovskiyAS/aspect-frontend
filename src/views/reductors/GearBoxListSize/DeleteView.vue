...
const data = ref<IDocument<IRedGearBoxListSize>>({ data: [], error: null, loading: true }) // Входы/Выходы
...
const submission = async () => {
  ...
  deleteDataReductors(`/data/RedGearBoxListSizes/${props.id}`)
    .then(() => {
      router.push('/reductors/RedGearBoxListSize/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
async function loadData() {
  data.value = await useFetch(`/data/RedGearBoxListSizes/${props.id}`, 'reductors')
  loading.value = false
}
...
<template>
  ...
  <h1 class="pt-5">RedGearBoxListSizes. Удалить?</h1>
