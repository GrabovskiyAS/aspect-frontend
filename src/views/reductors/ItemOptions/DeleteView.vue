...
const submission = async () => {
  ...
  deleteDataReductors(`/data/ItemOptions/${props.id}`)
    .then(() => {
      router.push('/reductors/ItemOptions/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
<template>
  ...
  <h1 class="pt-5">Опции Item. Удалить?</h1>
  ...
  <div v-else class="pt-5">
    ...
    <div class="field pt-5">
      <FloatLabel>
        <InputText
          id="oil_option_name"
          v-model="data.data[0].oil_option_name"
          disabled
          class="w-full"
        />
    ...
    <div class="field pt-5">
      <FloatLabel>
        <InputText
          ...
          v-model="data.data[0].color_option_name"
          disabled
          class="w-full"
        />
