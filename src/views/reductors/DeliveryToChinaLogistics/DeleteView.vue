...
const submission = async () => {
  ...
  deleteDataReductors(`/data/DeliveryToChinaLogistics/${props.id}`)
    .then(() => {
      router.push('/reductors/DeliveryToChinaLogistics/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
<template>
  ...
  <h1 class="pt-5">Доставка до логистического центра в Китае. Удалить?</h1>
