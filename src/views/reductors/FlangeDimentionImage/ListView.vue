...
const data = ref<IDocument<IFlangeDimentionImage>>({ data: [], error: null, loading: true })
const baseUrl = useBaseUrl()

onBeforeMount(async () => {
  data.value = await useFetch('/data/FlangeDimentionImages', 'reductors')
})
</script>
...
<template>
  ...
  <div v-else>
    <div class="grid">
      <div class="col-10">
        <h1 class="pt-5">FlangeDimentionImage</h1>
      </div>
        ...
        <Column header="Действия">
          <template #body="slotProps">
            <div class="flex gap-2">
              <RouterLink :to="`/reductors/FlangeDimentionImage/Update/${slotProps.data.id}`">
                <Button icon="pi pi-pencil" severity="info" />
              </RouterLink>
              <RouterLink :to="`/reductors/FlangeDimentionImage/Delete/${slotProps.data.id}`">
