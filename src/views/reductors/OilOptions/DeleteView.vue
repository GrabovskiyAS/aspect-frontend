...
const data = ref<IDocument<OilOptions>>({ data: [], error: null, loading: true }) // Входы/Выходы
...
const submission = async () => {
  ...
  deleteDataReductors(`/data/OilOptions/${props.id}`)
    .then(() => {
      router.push('/reductors/OilOptions/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
async function loadData() {
  data.value = await useFetch(`/data/OilOptions/${props.id}`, 'reductors')
  loading.value = false
}
...
<template>
  ...
  <h1 class="pt-5">Опции масла. Удалить?</h1>
