...
const data = ref<IDocument<OilOptions>>({ data: [], error: null, loading: true })

onBeforeMount(async () => {
  data.value = await useFetch('/data/OilOptions', 'reductors')
})
</script>
...
<template>
  ...
  <div v-else>
    <div class="grid">
      <div class="col-10">
        <h1 class="pt-5">Опции масла</h1>
      </div>
      <div class="col-2 flex justify-content-end flex-wrap">
        <RouterLink :to="`/reductors/OilOptions/Create/`" rel="noopener">
        ...
        <Column header="Действия">
          <template #body="slotProps">
            <div class="flex gap-2">
              <RouterLink :to="`/reductors/OilOptions/Update/${slotProps.data.id}`">
                <Button icon="pi pi-pencil" severity="info" />
              </RouterLink>
              <RouterLink :to="`/reductors/OilOptions/Delete/${slotProps.data.id}`">
