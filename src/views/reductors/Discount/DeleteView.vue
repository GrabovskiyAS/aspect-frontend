...
const data = ref<IDocument<RedDiscount>>({ data: [], error: null, loading: true }) // Входы/Выходы
...
const submission = async () => {
  ...
  deleteDataReductors(`/data/RedDiscounts/${props.id}`)
    .then(() => {
      router.push('/reductors/RedDiscount/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
<template>
  ...
  <h1 class="pt-5">Накрутка на редуктор с переходным фланцем. Удалить?</h1>
  ...
  <div v-else class="pt-5">
    ...
    <div class="field pt-5">
      <FloatLabel>
        <InputNumber id="discount" v-model="data.data[0].discount" disabled class="w-full" />
