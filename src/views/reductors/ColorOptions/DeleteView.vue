...
const data = ref<IDocument<ColorOptions>>({ data: [], error: null, loading: true }) // Входы/Выходы
...
const submission = async () => {
  ...
  deleteDataReductors(`/data/ColorOptions/${props.id}`)
    .then(() => {
      router.push('/reductors/ColorOptions/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
async function loadData() {
  data.value = await useFetch(`/data/ColorOptions/${props.id}`, 'reductors')
  loading.value = false
}
...
<template>
  ...
  <h1 class="pt-5">Опции покраски. Удалить?</h1>
