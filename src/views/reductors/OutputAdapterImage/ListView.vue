...
const data = ref<IDocument<IOutputAdapterImage>>({ data: [], error: null, loading: true })
const baseUrl = useBaseUrl()

onBeforeMount(async () => {
  data.value = await useFetch('/data/OutputAdapterImages', 'reductors')
})
</script>
...
<template>
  ...
  <div v-else>
    <div class="grid">
      <div class="col-10">
        <h1 class="pt-5">OutputAdapterImage</h1>
      </div>
        ...
        <Column header="Действия">
          <template #body="slotProps">
            <div class="flex gap-2">
              <RouterLink :to="`/reductors/OutputAdapterImage/Update/${slotProps.data.id}`">
                <Button icon="pi pi-pencil" severity="info" />
              </RouterLink>
              <RouterLink :to="`/reductors/OutputAdapterImage/Delete/${slotProps.data.id}`">
