...
const submission = async () => {
  ...
  insertDataReductors(`/data/DeliveryRussias`, payload)
    .then((response: AxiosResponse) => {
      router.push(`/reductors/DeliveryRussias/Update/${response.data.id}`)
    })
    .catch((error) => {
      console.log(error)
    })
...
<template>
  <h1 class="pt-5">Доставка по России. Создание</h1>
  <div class="pt-5">
