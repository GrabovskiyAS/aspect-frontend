...
const submission = async () => {
  ...
  deleteDataReductors(`/data/DeliveryRussias/${props.id}`)
    .then(() => {
      router.push('/reductors/DeliveryRussias/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
async function loadData() {
  data.value = await useFetch(`/data/DeliveryRussias/${props.id}`, 'reductors')
  loading.value = false
}
...
<template>
  ...
  <h1 class="pt-5">Доставка по России. Удалить?</h1>
  ...
  <div v-else class="pt-5">
    ...
    <div class="flex flex-wrap justify-center gap-4 pt-5">
      <RouterLink :to="`/reductors/DeliveryRussias/List`" rel="noopener">
