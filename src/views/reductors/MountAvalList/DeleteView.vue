...
const submission = async () => {
  ...
  deleteDataReductors(`/data/RedMountAvalLists/${props.id}`)
    .then(() => {
      router.push('/reductors/RedMountAvalList/List')
    })
    .catch((error) => {
      console.log(error)
    })
...
<template>
  ...
  <h1 class="pt-5">RedMountAvalList. Удалить?</h1>
  ...
  <div v-else class="pt-5">
    ...
    <div class="field pt-5">
      <FloatLabel>
        <InputText id="mount_types" v-model="data.data[0].mount_types" disabled class="w-full" />
