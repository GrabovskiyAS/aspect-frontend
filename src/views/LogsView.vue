...
import type { IDocument, ILog, ICompany, IUser, ISimpleDictionary } from '@/Interfaces/invertors'
import { getLoginByUserId } from '@/api/getCompanyNameByUserId'
...
const stacked = create2DArray(30, 10)
let dataset = create2DArray(30, 10)
...
const setChartData2 = () => {
  return { labels: dates.value, datasets: dataset }
}
  ...
  actions.value.data.map((action, index) => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      logs.value.data.map((log) => {
        if (
          ((selectedUser.value && log.user_id == selectedUser.value.id) || !selectedUser.value) &&
    ...
    dataset[index] = {
      type: 'bar',
      label: actions.value.data[index].name,
      backgroundColor: documentStyle.getPropertyValue(colors.value[index]),
      data: stacked[index],
    }
  })
        ...
        <TabPanel value="0">
          ...
          >
            ...
            <!-- <Column header="Организация" field="user">
              <template #body="{ data }">
                {{ data }}
                {{ getValueFromDictionary(companies.data, data.company_id) }}
              </template>
            </Column> -->
