...
import { ref } from 'vue'
import type { IInvOverload } from '@/Interfaces/invertors'
...
const submission = async () => {
  ...
  insertData(url, { g_mode: data.value.g_mode, p_mode: data.value.p_mode }).then((response) => {
    router.push(`/invertors/InvOverloads/Update/${response.data.id}`)
  })
  saving.value = false
}
</script>
