...
async function loadData() {
  let url: string = '/data/UserInvConfg'
  const userId = user.getUser().userId.value

  if (!user.isStaff()) url += `?user_id=${userId}` // загружаем только конфигурации пользователя

  data.value = await useFetch(url)
  options.value = await useFetch('/data/Inv_options')
  invertors.value = await useFetch('/data/Invertor_dict')
...
function confirm_delete(id: number) {
  confirm.require({
    ...
    accept: () => {
      deleteData(`/data/UserInvConfg/${id}`).then(() => {
        loadData()
        toast.add({
          severity: 'info',
    ...
    <div v-if="data.data.length > 0">
      ...
      >
        ...
        <Column header="Номер" field="id" sortable style="width: 15%">
          <template #body="{ data }"> {{ data.user_id }}/{{ data.id }} </template>
          <template #filter="{ filterModel, filterCallback }">
            <InputText
              v-model="filterModel.value"
              type="text"
