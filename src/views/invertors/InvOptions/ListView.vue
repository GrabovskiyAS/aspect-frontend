...
import type { IDocument, IInvOption, IInvSerie } from '@/Interfaces/invertors.js'
  ...
  <div v-else>
    <div class="grid">
      ...
      <div class="col-2 flex justify-content-end flex-wrap">
        <RouterLink :to="`/invertors/InvOptions/Create/`" rel="noopener">
    ...
    <div v-if="data.data.length > 0 && series.data.length > 0">
      ...
      >
        ...
        <Column field="option_type" header="Тип опции" sortable style="width: 10%">
          <template #body="{ data }">
            {{ getValueFromDictionary(optionType.data, data.option_id) }}
          </template>
          <template #filter="{ filterModel, filterCallback }">
            <Select
              v-model="filterModel.value"
              @change="filterCallback()"
              :options="optionTypeStr"
              placeholder="Серия"
        ...
        <Column header="Действия">
          <template #body="slotProps">
            <div class="flex gap-2">
              <RouterLink :to="`/invertors/InvOptions/Update/${slotProps.data.id}`">
                <Button icon="pi pi-pencil" severity="info" />
              </RouterLink>
              <RouterLink :to="`/invertors/InvOptions/Delete/${slotProps.data.id}`">
