...
import type { IDocument, IInvInputOuptput, ISimpleDictionary } from '@/Interfaces/invertors.js'
...
async function loadData() {
  ...
  signal.value = await useFetch('/data/Inv_type_of_signals')
  serie.value = await useFetch('/data/Inv_series_dict')
  loading.value = false
}
  ...
  <div v-else>
    <div class="grid">
      ...
      <div class="col-2 flex justify-content-end flex-wrap">
        <RouterLink :to="`/invertors/InputOutput/Create/`" rel="noopener">
        ...
        <Column header="Действия">
          <template #body="slotProps">
            <div class="flex gap-2">
              <RouterLink :to="`/invertors/InputOutput/Update/${slotProps.data.id}`">
                <Button icon="pi pi-pencil" severity="info" />
              </RouterLink>
              <RouterLink :to="`/invertors/InputOutput/Delete/${slotProps.data.id}`">
