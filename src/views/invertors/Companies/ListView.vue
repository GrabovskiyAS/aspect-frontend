...
import type { IDocument, ICompany } from '@/Interfaces/invertors.js'
  ...
  <div v-else>
    <div class="grid">
      ...
      <div class="col-2 flex justify-content-end flex-wrap">
        <RouterLink :to="`/invertors/Companies/Create/`" rel="noopener">
    ...
    <div v-if="data.data">
      ...
      >
        <Column field="logo" header="" style="width: 10%">
          <template #body="{ data }">
            <img
              v-if="data.logo.length > 4"
              :src="`${baseUrl.s3Storage}/${data.logo}`"
              width="100"
            />
        ...
        <Column header="Действия">
          <template #body="slotProps">
            <div class="flex gap-2">
              <RouterLink :to="`/invertors/Companies/Update/${slotProps.data.id}`">
                <Button icon="pi pi-pencil" severity="info" />
              </RouterLink>
              <RouterLink :to="`/invertors/Companies/Delete/${slotProps.data.id}`">
