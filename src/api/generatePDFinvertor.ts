...
export async function generatePDF(
  ...
  await axios.get(`${baseUrl.s3url}/dms/getBase64/link_to_doc.png`).then((response) => {
    docs = response.data
  })
  await axios.get(`${baseUrl.s3url}/dms/getBase64/${serie.photo}`).then((response) => {
    photo = response.data
  })
  await axios.get(`${baseUrl.s3url}/dms/getBase64/${serie.schema}`).then((response) => {
    schema = response.data
  })

  let photoRatio: number = 1
  let schemaRatio: number = 1

  getMeta(photo, (err: any, img: any) => {
    photoRatio = img.naturalWidth / img.naturalHeight
  })
  getMeta(schema, (err: any, img: any) => {
    schemaRatio = img.naturalWidth / img.naturalHeight
  })
  ...
  ambientTemperature.value = await useFetch(
    `/data/Ambient_temperatures/${serie.ambient_temperature_id}`,
  )
  ...
  const signals_table_body: Array<[string, string]> = [['', '']]
  ...
  const options_table_body_w_price: Array<[string, string, string, string, string]> = [
    ['', '', '', '', ''],
  ]
  const options_table_body_wo_price: Array<[string, string, string, string]> = [['', '', '', '']]

  const invertor_table_header = ['Параметр', 'Значение']
  const invertor_table_body: Array<[string, string]> = []

  invertor_table_body.push(['Тип оборудования', 'Преобразователь частоты'])
  if (print_price) {
    invertor_table_body.push([
      'Цена преобразователя (с НДС)',
      priceFormat(Number(invertor.price)) + ' руб.',
    ])
...
Режим P: ` +
  ...
  invertor_table_body.push([
    ...
    'Режиим G: ' +
      invertor.overload_g_mode +
      `
Режим P: ` +
      invertor.overload_p_mode +
      `
(не  чаще 1 раза в 10 мин)`,
  ...
  invertor_table_body.push([
    ...
    invertor.type_of_accuracy_freq_str || '',
  ])
  ...
  // console.log(signals)
  signals.forEach(
    (item: IInvSignalInputOutput, index: number) =>
  ...
  optionsSelected.forEach((item: IInvOption, index: number) => {
    if (print_price)
      options_table_body_w_price[index] = [
        ...
        calcPrice(Number(item.price), Number(item.discount)).toString(),
      ]
    // Цена
    else
