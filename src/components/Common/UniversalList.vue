...
import type { IDocument, ISimpleDictionary } from '@/Interfaces/invertors.js'
...
const search = () => {
  if (searchText.value.length > 0) {
    const searchData = data.value.data.filter(
      (item) =>
        item.name.toLowerCase().includes(searchText.value.toLowerCase()) ||
        item.id!.toString().includes(searchText.value.toLowerCase()),
    )
  ...
  <div v-else>
    <div class="grid">
      ...
      <div class="col-2 flex justify-content-end flex-wrap">
        <RouterLink :to="`/invertors/${path}/Create/`" rel="noopener">
      ...
      <Column header="Действия">
        <template #body="slotProps">
          <div class="flex gap-2">
            <RouterLink :to="`/invertors/${path}/Update/${slotProps.data.id}`">
              <Button icon="pi pi-pencil" severity="info" />
            </RouterLink>
            <RouterLink :to="`/invertors/${path}/Delete/${slotProps.data.id}`">
